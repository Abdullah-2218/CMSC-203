...
public class CircusDriverAppGUI extends Application {
 ...
 // ===================== DISPLAY ANIMALS WITH IMAGES =====================
    ...
    public double calculateTicketPrice(String dayOfWeek, double basePrice, String seatLevel, double customerDiscount) {
        double finalPrice = basePrice;

        DayOfWeek dayEnum = DayOfWeek.valueOf(dayOfWeek.toUpperCase());

        // Apply ONLY weekday discount if no customer discount
        if (customerDiscount == 0 && dayEnum.getDiscount() > 0) {
            finalPrice *= (1 - dayEnum.getDiscount());
        }

        // Apply ONLY customer discount (ignore others)
        if (customerDiscount > 0) {
            finalPrice *= (1 - customerDiscount);
            return finalPrice; // stop here
        }

        // Apply seat multiplier ONLY if no discounts
        if (customerDiscount == 0 && dayEnum.getDiscount() == 0) {
            switch (seatLevel) {
                case "T-Level" -> finalPrice *= 2;
                case "Lower Level" -> finalPrice *= 1.5;
            }
        }
    ...
    public String getSingleTicketDetails(String dayOfWeek, double basePrice, String seatLevel, double customerDiscount) {
